...
$object = new clearance_form($connect);

$object->collectUserID();
$object->selectStudentDetails();

// If a Document is submitted
if (isset($_POST["upload_document"])) {
  $object->collectFormInputs();

  $isImagePassed = $object->imageProcessing();
  if ($isImagePassed) {
    $object->insertIntoDB();
  }
}
...
<head>
  ...
  <style>
    ...
    .clearance-form-section h4 {
      color: #007bff;
      margin-bottom: 1.5rem;
    }

    .clearance-form-section table {
      width: 100%;
    }

    .clearance-form-section .document-row {
      display: flex;
      align-items: center;
    ...
    .clearance-form-section .document-row .document-name {
      flex-grow: 1;
    }

    .clearance-form-section .view-btn,
    .clearance-form-section .delete-btn {
      cursor: pointer;
      margin: 0 0.5rem;
      color: #007bff;
    }
    ...
    .clearance-form-section .upload-btn {
    ...
    .clearance-form-section .upload-btn:hover {
  ...
  <aside
    ...
    <div
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="./clearance_form.php">
            <div
              class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
              <svg
                width="12px"
            ...
            <span class="nav-link-text ms-1">Submit Clearance Form</span>
          </a>
        </li>
        <!--<li class="nav-item">
          <a class="nav-link" href="./view_notifications.php">
            ...
            <span class="nav-link-text ms-1">View Notifications</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="./download_letter.php">
            <div
              class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
              <svg
                width="12px"
            ...
            <span class="nav-link-text ms-1">Download Clearance Letter</span>
          </a>
        </li>
        <li class="nav-item">
            ...
            <div class="card-body p-3">
              <div class="row">
                ...
                <div class="container mt-5">
                  <h2 class="text-center mb-4">Submit Clearance Documents</h2>

                  <!-- Selection Area for Department, Faculty, Admin Block -->
                  <div class="mb-4 text-center">
                    <a href="<?php echo $_SERVER["PHP_SELF"]; ?>?role_id=4" class="btn btn-primary mx-2">Submit to Department</a>
                    <a href="<?php echo $_SERVER["PHP_SELF"]; ?>?role_id=3" class="btn btn-success mx-2">Submit to Faculty</a>
                    <a href="<?php echo $_SERVER["PHP_SELF"]; ?>?role_id=2" class="btn btn-info mx-2">Submit to Admin Block</a>
                  </div>

                  <?php
                  if (isset($_GET["role_id"])) {
                    $sql = $object->selectRequirements();
                    while ($row = $sql->fetch_assoc()) {
                  ?>
                      <!-- Clearance Form Container -->
                      <div class="clearance-form-section mt-4" id="clearanceFormSection">
                        <h4>Submit Clearance for <?php echo $row["role_name"]; ?></h4>
                          ...
                          <tbody id="documentList">
                            <tr class="document-row">
                              <td class="document-name">
                                <?php echo $row["requirement_title"]; ?> (<?php echo $row["requirement_description"]; ?>)
                              </td>
                              <td>
                                <form action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="POST" enctype="multipart/form-data">
                                  <input type="hidden" name="requirement_id" value="<?php echo $row["requirement_id"]; ?>">
                                  <div class="input-group">
                                    <input type="file" name="document" class="form-control" accept=".pdf,.jpg,.png,.jpeg" required />
                                    <button type="submit" name="upload_document" class="upload-btn input-group-text">Upload</button>
                                  </div>
                                </form>
                              </td>
                              <!-- <td>
